...
router.get("/tenant/dashboard", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    SELECT 
      ...
      l.grace_period_days,
      l.late_fee,
      -- ADD THIS: Get current month's utilities
      (
        SELECT COALESCE(SUM(utilities_charges), 0)
        FROM rent_payments
        WHERE lease_id = l.id
...
`;
    ...
    const balanceQuery = `
  SELECT 
    COALESCE(SUM(CASE WHEN rp.payment_status IN ('overdue', 'pending') THEN (rp.amount_due + COALESCE(rp.utilities_charges, 0) + COALESCE(rp.late_fee, 0)) - rp.amount_paid END), 0) as current_balance,
...
`;
    ...
    const maintenanceRequests = await client.query(maintenanceQuery, [
      tenantId,
    ]);
    ...
    const paymentSubmissions = await client.query(paymentSubmissionsQuery, [
      tenantId,
    ]);

    const tenantDocumentsResult = await client.query(tenantDocumentsQuery, [
      tenantId,
    ]);
    ...
    const tenantData = {
      tenant: {
        ...
        utilitiesAmount: parseFloat(tenant.current_utilities) || 0, // ADD THIS
      ...
      rentHistory: rentHistory.rows.map((payment) => ({
        ...
        utilityBreakdown: payment.utility_breakdown || null,
      ...
      stats: {
        ...
        unreadNotifications: notifications.rows.filter((n) => !n.is_read)
          .length,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const { amount, paymentMethod, reference, transactionDate, notes } =
        req.body;
      ...
      if (!tenantId) {
        return res.status(403).json({
          status: 403,
          message:
            "Access denied. User is not associated with a tenant account.",
        });
      }
      ...
      `;
        ...
        res.status(200).json({
          ...
          data: {
            ...
            leaseNumber,
            verificationNote:
              "All payments require admin verification within 24 hours",
          },
        });
      } finally {
        client.release();
