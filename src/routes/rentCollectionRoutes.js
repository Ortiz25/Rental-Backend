...
router.get('/rent-collection', authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const countQuery = `
      ...
      JOIN units u ON l.unit_id = u.id
      JOIN properties p ON u.property_id = p.id
      WHERE ${whereClause.replace(`LIMIT $${paramCount + 1} OFFSET $${paramCount + 2}`, '')}
    `;
...
router.get('/rent-collection/summary', authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const summaryQuery = `
      WITH rent_summary AS (
        SELECT 
          COALESCE(SUM(amount_due + COALESCE(late_fee, 0)), 0) as total_due,
          COALESCE(SUM(amount_paid), 0) as total_collected,
          COALESCE(SUM(CASE WHEN payment_status = 'pending' THEN amount_due ELSE 0 END), 0) as total_pending,
          COALESCE(SUM(CASE WHEN payment_status = 'overdue' THEN amount_due - amount_paid ELSE 0 END), 0) as total_overdue,
    ...
    const result = await client.query(summaryQuery, [dateRange.start, dateRange.end]);
    ...
    const metricsResult = await client.query(metricsQuery, [dateRange.start, dateRange.end]);
    ...
    const responseData = {
      ...
      pending_verification_amount: parseFloat(summary.pending_verification_amount) || 0,
      verified_amount_this_month: parseFloat(summary.verified_amount_this_month) || 0,
      avg_days_to_pay: parseFloat(metrics.avg_days_to_pay) || 0,
      collection_rate: parseFloat(summary.collection_rate) || 0,
      payment_completion_rate: parseFloat(summary.payment_completion_rate) || 0,
      late_payment_rate: parseFloat(metrics.late_payment_rate) || 0,
      pending_verifications: parseInt(summary.pending_verifications) || 0,
      verifications_this_month: parseInt(summary.verifications_this_month) || 0
    };
...
router.put('/rent-collection/:id/process', authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    if (!allowedRoles.includes(req.user.role)) {
      return res.status(403).json({
        status: 403,
        message: 'Access denied. Insufficient privileges for payment processing.',
        data: null
      });
    }
    ...
    const newTotalPaid = parseFloat(currentPayment.amount_paid) + parseFloat(amount_paid);
    const totalDue = parseFloat(currentPayment.amount_due) + parseFloat(currentPayment.late_fee || 0);
    ...
    console.log(`✅ Payment processed successfully for ${currentPayment.tenant_name}`);
...
router.post('/rent-collection/generate', authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const existingResult = await client.query(existingQuery, [dateRange.start, dateRange.end]);
    ...
    leasesResult.rows.forEach((lease) => {
      ...
      const dueDate = new Date(year, month - 1, Math.min(dueDay, new Date(year, month, 0).getDate()));
      
      paramCount += 4;
      paymentInserts.push(`($${paramCount - 3}, $${paramCount - 2}, $${paramCount - 1}, $${paramCount})`);
      values.push(
        lease.lease_id,
    ...
    console.log(`✅ Generated ${insertResult.rows.length} rent payment records`);
...
router.post('/rent-collection/update-overdue', authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ Updated ${updateResult.rows.length} payments to overdue status`);
