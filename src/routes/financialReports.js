...
router.get('/summary', authenticateTokenSimple, async (req, res) => {
    ...
    try {
        ...
        const expensesQuery = `
            SELECT 
                COALESCE(SUM(actual_cost), 0) as total_expenses,
                COALESCE(SUM(CASE WHEN completed_date >= CURRENT_DATE - INTERVAL '30 days' THEN actual_cost END), 0) as expenses_last_30_days,
                COALESCE(SUM(CASE WHEN completed_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month') 
                    AND completed_date < DATE_TRUNC('month', CURRENT_DATE) THEN actual_cost END), 0) as expenses_previous_month
        ...
        const maintenanceQuery = `
            SELECT 
                COALESCE(SUM(actual_cost), 0) as maintenance_costs
            FROM maintenance_requests
        ...
        const maintenanceExpenseQuery = expensesQuery.replace('$1', queryParams.length > 0 ? '$1' : 'NULL').replace('$2', queryParams.length > 0 ? '$2' : 'NULL');

        const [revenueResult, expensesResult, occupancyResult, pendingResult, maintenanceResult] = await Promise.all([
        ...
        const revenueChange = revenueLastMonth > 0 ? 
            ((revenue - revenueLastMonth) / revenueLastMonth * 100).toFixed(1) : 0;
        const expenseChange = expensesLastMonth > 0 ? 
            ((expenses - expensesLastMonth) / expensesLastMonth * 100).toFixed(1) : 0;
    ...
    try {
        ...
        const expenseQuery = `
            SELECT 
                ...
                SUM(actual_cost) as expenses
            FROM maintenance_requests
            WHERE actual_cost IS NOT NULL AND completed_date IS NOT NULL
            AND completed_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${months} months')
        ...
        const maintenanceQuery = `
            SELECT 
                ...
                SUM(CASE WHEN category IN ('Maintenance', 'Plumbing', 'Electrical', 'HVAC', 'Structural') 
                    THEN actual_cost ELSE 0 END) as maintenance,
                SUM(CASE WHEN category IN ('Utilities', 'Cleaning') 
                    THEN actual_cost ELSE 0 END) as utilities
            FROM maintenance_requests
            WHERE actual_cost IS NOT NULL AND completed_date IS NOT NULL
            AND completed_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${months} months')
            GROUP BY DATE_TRUNC('month', completed_date)
        ...
        // Execute queries
        const [monthSeriesResult, revenueResult, expenseResult, maintenanceResult] = await Promise.all([
        ...
        revenueResult.rows.forEach(row => {
            revenueMap.set(row.month_date.getTime(), parseFloat(row.revenue) || 0);
        });

        const expenseMap = new Map();
        expenseResult.rows.forEach(row => {
            expenseMap.set(row.month_date.getTime(), parseFloat(row.expenses) || 0);
        });

        const maintenanceMap = new Map();
        const utilitiesMap = new Map();
        maintenanceResult.rows.forEach(row => {
            const key = row.month_date.getTime();
        ...
        const monthlyData = monthSeriesResult.rows.map(row => {
            const monthKey = row.date_month.getTime();
            return {
                month: row.month,
                revenue: revenueMap.get(monthKey) || 0,
                expenses: expenseMap.get(monthKey) || 0,
                maintenance: maintenanceMap.get(monthKey) || 0,
                utilities: utilitiesMap.get(monthKey) || 0
            };
        });
...
router.post('/generate-report', authenticateTokenSimple, async (req, res) => {
    ...
    try {
        const { startDate, endDate, type = 'detailed', includeCharts = true } = req.body;
        ...
        const reportQuery = `
            ...
            expense_summary AS (
                SELECT 
                    ...
                    COALESCE(AVG(actual_cost), 0) as avg_expense
                FROM maintenance_requests
                WHERE actual_cost IS NOT NULL 
        ...
        const report = {
            ...
            propertyPerformance: propertyPerformance.map(prop => ({
                ...
                occupancyRate: prop.total_units > 0 ? 
...
router.get('/export', authenticateTokenSimple, async (req, res) => {
    ...
    try {
        ...
        const exportQuery = `
            SELECT 
                rp.payment_date::DATE as payment_date,
    ...
    try {
        ...
        const analyticsQuery = `
            ...
            expense_analytics AS (
                SELECT 
                    ...
                    SUM(CASE WHEN priority = 'emergency' THEN actual_cost ELSE 0 END) as emergency_costs,
                    COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_requests
                FROM maintenance_requests
                WHERE actual_cost IS NOT NULL
                AND completed_date >= DATE_TRUNC('${period}', CURRENT_DATE)
            ),
        ...
        const analyticsData = {
            ...
            expenseMetrics: {
                ...
                completionRate: analytics.total_maintenance_requests > 0 ? 
                    Math.round((analytics.completed_requests / analytics.total_maintenance_requests) * 100) : 0
            },
            propertyMetrics: {
    ...
    try {
        ...
        const performanceQuery = `
            SELECT 
                ...
                COALESCE(SUM(mr.actual_cost), 0) as maintenance_costs,
            ...
            LEFT JOIN maintenance_requests mr ON u.id = mr.unit_id 
                ...
                AND mr.completed_date >= DATE_TRUNC('${period}', CURRENT_DATE)
        ...
        const propertyPerformance = result.rows.map(row => ({
            ...
            profitMargin: row.total_revenue > 0 ? 
                Math.round((row.net_income / row.total_revenue) * 100) : 0
        }));
