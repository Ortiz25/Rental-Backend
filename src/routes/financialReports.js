...
router.get("/monthly-data", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    // Generate month series using string interpolation for months to avoid type inference issues
    const monthSeriesQuery = `
      ...
      FROM (
        SELECT 
          DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month' * generate_series(0, ${monthsInt} - 1)) as date_month
      ) month_series
      ORDER BY date_month
    `;

    // Revenue data with property filter
    const revenueQuery = `
      SELECT 
        DATE_TRUNC('month', rp.payment_date) as month_date,
        SUM(rp.amount_paid) as revenue
      FROM rent_payments rp
      LEFT JOIN leases l ON rp.lease_id = l.id
      LEFT JOIN units u ON l.unit_id = u.id
      WHERE rp.payment_status = 'paid'
      AND rp.payment_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${monthsInt} months')
      ${propertyFilter}
      GROUP BY DATE_TRUNC('month', rp.payment_date)
    `;

    // Expense data with property filter
    const expenseQuery = `
      SELECT 
        DATE_TRUNC('month', COALESCE(mr.completed_date, mr.requested_date)) as month_date,
        SUM(COALESCE(mr.actual_cost, mr.estimated_cost)) as expenses
      FROM maintenance_requests mr
      LEFT JOIN units u ON mr.unit_id = u.id
      WHERE (mr.actual_cost IS NOT NULL OR mr.estimated_cost IS NOT NULL)
      AND COALESCE(mr.completed_date, mr.requested_date) >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${monthsInt} months')
      ${propertyFilter}
      GROUP BY DATE_TRUNC('month', COALESCE(mr.completed_date, mr.requested_date))
    `;

    // Maintenance breakdown with property filter
    const maintenanceQuery = `
      SELECT 
        DATE_TRUNC('month', COALESCE(mr.completed_date, mr.requested_date)) as month_date,
        SUM(CASE WHEN mr.category IN ('Maintenance', 'Plumbing', 'Electrical', 'HVAC', 'Structural') 
            THEN COALESCE(mr.actual_cost, mr.estimated_cost) ELSE 0 END) as maintenance,
        SUM(CASE WHEN mr.category IN ('Utilities', 'Cleaning') 
            THEN COALESCE(mr.actual_cost, mr.estimated_cost) ELSE 0 END) as utilities
      FROM maintenance_requests mr
      LEFT JOIN units u ON mr.unit_id = u.id
      WHERE (mr.actual_cost IS NOT NULL OR mr.estimated_cost IS NOT NULL)
      AND COALESCE(mr.completed_date, mr.requested_date) >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${monthsInt} months')
    ...
    // Execute queries - monthSeriesQuery doesn't need parameters now
    const [monthSeriesResult, revenueResult, expenseResult, maintenanceResult] =
      await Promise.all([
        client.query(monthSeriesQuery), // No parameters needed
        client.query(revenueQuery, queryParams),
        client.query(expenseQuery, queryParams),
        client.query(maintenanceQuery, queryParams),
      ]);
...
router.get("/expense-breakdown", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    // Date filtering
    if (period === "month") {
      dateFilter = `AND COALESCE(mr.completed_date, mr.requested_date) >= DATE_TRUNC('month', CURRENT_DATE)`;
    } else if (period === "quarter") {
      dateFilter = `AND COALESCE(mr.completed_date, mr.requested_date) >= DATE_TRUNC('quarter', CURRENT_DATE)`;
    } else if (period === "year") {
      dateFilter = `AND COALESCE(mr.completed_date, mr.requested_date) >= DATE_TRUNC('year', CURRENT_DATE)`;
    }
    ...
    const expenseBreakdownQuery = `
      WITH maintenance_expenses AS (
        SELECT 
          COALESCE(mr.category, 'Other') as name,
          SUM(COALESCE(mr.actual_cost, mr.estimated_cost)) as value
        FROM maintenance_requests mr
        LEFT JOIN units u ON mr.unit_id = u.id
        WHERE (mr.actual_cost IS NOT NULL OR mr.estimated_cost IS NOT NULL)
        ${dateFilter}
        ${propertyFilter}
        GROUP BY COALESCE(mr.category, 'Other')
        HAVING SUM(COALESCE(mr.actual_cost, mr.estimated_cost)) > 0
      ),
...
router.get("/recent-transactions", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const transactionsQuery = `
      ...
      SELECT 
        id, date, description, type, amount, category, payment_method
      FROM (
...
router.get("/analytics", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    const { period = "year", propertyId } = req.query;
    ...
    const analyticsQuery = `
      ...
      expense_analytics AS (
        SELECT 
          ...
          AVG(COALESCE(mr.actual_cost, mr.estimated_cost)) as avg_maintenance_cost,
          SUM(CASE WHEN mr.priority = 'emergency' THEN COALESCE(mr.actual_cost, mr.estimated_cost) ELSE 0 END) as emergency_costs,
    ...
    const result = await client.query(analyticsQuery, queryParams);
...
router.get("/payment-trends", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    const { months = 6, propertyId } = req.query;
    const monthsInt = parseInt(months);
    ...
    const trendsQuery = `
      WITH monthly_trends AS (
        SELECT 
          ...
          TO_CHAR(rp.payment_date, 'Mon YYYY') as month_label,
          COUNT(*) as total_payments,
          SUM(rp.amount_paid) as total_revenue,
          AVG(rp.amount_paid) as avg_payment,
          COUNT(CASE WHEN rp.payment_date <= rp.due_date THEN 1 END) as on_time_count,
          COUNT(CASE WHEN rp.payment_date > rp.due_date THEN 1 END) as late_count,
          SUM(rp.late_fee) as total_late_fees
        FROM rent_payments rp
        LEFT JOIN leases l ON rp.lease_id = l.id
        LEFT JOIN units u ON l.unit_id = u.id
        WHERE rp.payment_status = 'paid' 
        AND rp.payment_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '${monthsInt} months')
        ${propertyFilter}
        GROUP BY TO_CHAR(rp.payment_date, 'YYYY-MM'), TO_CHAR(rp.payment_date, 'Mon YYYY')
        ORDER BY month
      )
      SELECT 
        month_label, total_payments, total_revenue, avg_payment,
        on_time_count, late_count, total_late_fees,
        CASE 
    ...
    const result = await client.query(trendsQuery, queryParams);
  ...
  try {
    ...
    const performanceQuery = `
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN u.occupancy_status = 'vacant' THEN u.id END) as vacant_units,
        COALESCE(SUM(rp.amount_paid), 0) as total_revenue,
        ...
        CASE 
          WHEN COUNT(DISTINCT u.id) > 0 THEN 
            ROUND((COUNT(DISTINCT CASE WHEN u.occupancy_status = 'occupied' THEN u.id END)::DECIMAL / COUNT(DISTINCT u.id)) * 100, 2)
          ELSE 0
        END as occupancy_rate,
        COALESCE(SUM(rp.amount_paid), 0) - COALESCE(SUM(COALESCE(mr.actual_cost, mr.estimated_cost)), 0) as net_income
      FROM properties p
...
router.post("/generate-report", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const reportQuery = `
      WITH revenue_summary AS (
        SELECT 
          ...
          COALESCE(AVG(rp.amount_paid), 0) as avg_payment
        FROM rent_payments rp
        LEFT JOIN leases l ON rp.lease_id = l.id
        LEFT JOIN units u ON l.unit_id = u.id
        WHERE rp.payment_status = 'paid' AND rp.payment_date BETWEEN $1 AND $2
        ${propertyFilter}
      ),
      expense_summary AS (
        SELECT 
          ...
          COALESCE(AVG(COALESCE(mr.actual_cost, mr.estimated_cost)), 0) as avg_expense
        FROM maintenance_requests mr
        LEFT JOIN units u ON mr.unit_id = u.id
        WHERE (mr.actual_cost IS NOT NULL OR mr.estimated_cost IS NOT NULL)
        AND COALESCE(mr.completed_date, mr.requested_date) BETWEEN $1 AND $2
        ${propertyFilter}
      )
      SELECT 
        rs.total_revenue, rs.payment_count, rs.avg_payment,
        es.total_expenses, es.expense_count, es.avg_expense,
    ...
    const propertyQuery = `
      ...
      LEFT JOIN leases l ON u.id = l.unit_id AND l.lease_status = 'active'
      LEFT JOIN rent_payments rp ON l.id = rp.lease_id 
        AND rp.payment_status = 'paid' AND rp.payment_date BETWEEN $1 AND $2
      ${propertyId !== "all" ? "WHERE p.id = $3" : ""}
      GROUP BY p.id, p.property_name
      ORDER BY property_revenue DESC
    `;

    const [reportResult, propertyResult] = await Promise.all([
      client.query(reportQuery, params),
      client.query(propertyQuery, params),
    ]);
    ...
    const report = {
      ...
      propertyPerformance: propertyPerformance.map((prop) => ({
        ...
        occupancyRate: parseFloat(prop.occupancy_rate) || 0,
      })),
    };
...
router.get("/export", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const exportQuery = `
      SELECT mr.requested_date::DATE as request_date,
        COALESCE(mr.completed_date::DATE::TEXT, 'Pending') as completion_date,
        COALESCE(p.property_name, 'Unknown') as property_name,
        COALESCE(u.unit_number, 'N/A') as unit_number,
        mr.request_title, mr.category, mr.priority, mr.status,
      ...
      AND (mr.actual_cost IS NOT NULL OR mr.estimated_cost IS NOT NULL)
      ORDER BY mr.requested_date DESC LIMIT 1000
    `;
...
router.get("/property-expenses", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    let query = `
      SELECT pe.id, pe.property_id, p.property_name, pe.expense_type, pe.amount, pe.frequency,
        pe.start_date, pe.end_date, pe.is_active, pe.description, pe.created_at, pe.updated_at,
        ec.category_name,
        CASE WHEN pe.frequency = 'monthly' THEN pe.amount
          WHEN pe.frequency = 'quarterly' THEN pe.amount / 3
          WHEN pe.frequency = 'annual' THEN pe.amount / 12 ELSE pe.amount END as monthly_equivalent
      FROM property_expenses pe
...
router.post("/property-expenses", authenticateTokenSimple, async (req, res) => {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO property_expenses (property_id, expense_type, amount, frequency, start_date, end_date, description, category_id, created_by)
...
router.get("/expense-categories", authenticateTokenSimple, async (req, res) => {
  const client = await pool.connect();
  try {
    const query = `SELECT id, category_name, description, is_active, created_at 
      FROM expense_categories WHERE is_active = true ORDER BY category_name`;
